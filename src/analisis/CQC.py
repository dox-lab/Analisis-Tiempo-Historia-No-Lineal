...
class AnalisisModalEspectral:
    ...
    def _coeficiente_correlacion(self, T_i, T_j, damping_ratio=0.05):
        """
        Calcula el coeficiente de correlación para la combinación cuadrática completa (CQC).
        :param T_i: Periodo del modo i.
        :param T_j: Periodo del modo j.
        :param damping_ratio: Relación de amortiguamiento (default = 0.05 para amortiguamiento del 5%).
        :return: Coeficiente de correlación.
        """
        beta_ij = T_j / T_i
        num = 8 * damping_ratio**2 * (1 + beta_ij) * beta_ij
        den = (1 - beta_ij**2)**2 + 4 * damping_ratio**2 * beta_ij * (1 + beta_ij)**2
        return num / den

    def combinacion_cqc(self, respuestas_modales):
        """
        Realiza la combinación modal utilizando el método de Combinación Cuadrática Completa (CQC).
        :param respuestas_modales: Respuestas modales (desplazamientos o fuerzas).
        :return: Combinación modal utilizando CQC.
        """
        respuesta_total = 0
        for i in range(self.num_modos):
            for j in range(self.num_modos):
                rho_ij = self._coeficiente_correlacion(self.T[i], self.T[j])
                respuesta_total += rho_ij * respuestas_modales[i] * respuestas_modales[j]
        return np.sqrt(respuesta_total)
    ...
    def combinacion_modal(self):
        ...
        # Evitar valores negativos en las respuestas
        disp_comb_x = 0.25 * disp_abs_x + 0.75 * np.sqrt(np.maximum(disp_rmsc_x, 0))
        drift_comb_x = 0.25 * drift_abs_x + 0.75 * np.sqrt(np.maximum(drift_rmsc_x, 0))
        shear_comb_x = 0.25 * shear_abs_x + 0.75 * np.sqrt(np.maximum(shear_rmsc_x, 0))

        disp_comb_y = 0.25 * disp_abs_y + 0.75 * np.sqrt(np.maximum(disp_rmsc_y, 0))
        drift_comb_y = 0.25 * drift_abs_y + 0.75 * np.sqrt(np.maximum(drift_rmsc_y, 0))
        shear_comb_y = 0.25 * shear_abs_y + 0.75 * np.sqrt(np.maximum(shear_rmsc_y, 0))
